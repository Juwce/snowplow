...
trait Adapter {
  ...
  private[registry] def cleanupJsonEventValues(
    ...
    def toStringField(seconds: Long): String = {
      val dt: DateTime = new DateTime(seconds * 1000)
      JsonSchemaDateTimeFormat.print(dt)
    }

    val intToDateString: Kleisli[Option, Json, Json] = Kleisli(
      (json: Json) =>
        json
          .as[Int]
          .toOption
          .map { v =>
            Either
              .catchNonFatal(Json.fromString(toStringField(v.longValue)))
              .getOrElse(Json.fromInt(v))
        })

    val stringToDateString: Kleisli[Option, Json, Json] = Kleisli(
      (json: Json) =>
        json
          .as[String]
          .toOption
          .map { v =>
            Either
              .catchNonFatal(Json.fromString(toStringField(v.toLong)))
    ...
    (for {
      ...
      newValue = jObj.kleisli
        .andThen(intToDateString)
        .andThen(stringToDateString)
        .run(tsFieldKey)
      newJObj = newValue.map(v => jObj.add(tsFieldKey, v)).getOrElse(jObj)
      res = eventOpt match {
        case Some((keyName, _)) => newJObj.remove(keyName)
        case None => newJObj
      }
