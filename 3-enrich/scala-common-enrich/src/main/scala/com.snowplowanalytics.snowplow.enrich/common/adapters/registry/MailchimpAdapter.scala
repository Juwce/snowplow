...
object MailchimpAdapter extends Adapter {
   ...
   */
  private[registry] def toJFields(parameters: RawEventParameters): List[(String, Json)] =
    for {
   ...
   */
  private[registry] def toNestedJField(keys: NonEmptyList[String], value: String): (String, Json) =
    keys.toList match {
      case h1 :: h2 :: t => (h1, Json.obj(toNestedJField(NonEmptyList(h2, t: _*), value)))
   ...
   */
  private[registry] def mergeJFields(jfields: List[(String, Json)]): Json =
    jfields match {
