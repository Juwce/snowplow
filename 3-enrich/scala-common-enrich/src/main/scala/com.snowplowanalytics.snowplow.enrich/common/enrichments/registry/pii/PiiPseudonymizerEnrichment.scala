...
object PiiPseudonymizerEnrichment extends ParseableEnrichment {
  ...
  def parse(config: Json, schemaKey: SchemaKey): ValidatedNelMessage[PiiPseudonymizerEnrichment] = {
    for {
      ...
      emitIdentificationEvent = ScalazCirceUtils.extract[Boolean](conf, "emitEvent").toOption
        .getOrElse(false)
      piiFields <- ScalazCirceUtils.extract[List[Json]](conf, "parameters", "pii")
  ...
  private def extractFields(piiFields: List[Json]): Validation[String, List[PiiField]] =
    piiFields.map { json =>
      extractString(json, "pojo", "field").flatMap(extractPiiScalarField)
        .orElse {
          json.hcursor.downField("json").focus.toSuccess("No json field")
  ...
  private def extractStrategy(config: Json): Validation[String, PiiStrategyPseudonymize] =
    ScalazCirceUtils.extract[PiiStrategyPseudonymize](config, "parameters", "strategy")
      .leftMap(_.getMessage)
...
final case class PiiJson(fieldMutator: Mutator, schemaCriterion: SchemaCriterion, jsonPath: String)
  ...
  override def applyStrategy(
    ...
    (for {
      ...
      (substituted, modifiedFields) = parsed.asObject.map { obj =>
        val jObjectMap = obj.toMap
        val contextMapped = jObjectMap.map(mapContextTopFields(_, strategy))
        (Json.obj(contextMapped.mapValues(_._1).toList: _*), contextMapped.values.map(_._2).flatten)
