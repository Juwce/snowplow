...
object IpLookupsEnrichment extends ParseableEnrichment {
  ...
  def parse(
    ...
    isParseable(config, schemaKey).flatMap { conf =>
      def db(name: String) = getArgumentFromName(conf, name)
      (db("geo") |@| db("isp") |@| db("domain") |@| db("connectionType")) {
        case (geo, isp, domain, ct) =>
          IpLookupsEnrichment(geo.some, isp.some, domain.some, ct.some, localMode)
      }
    }
  ...
  private def getArgumentFromName(
    conf: Json,
    name: String
  ): ValidatedNelMessage[(String, URI, String)] = {
