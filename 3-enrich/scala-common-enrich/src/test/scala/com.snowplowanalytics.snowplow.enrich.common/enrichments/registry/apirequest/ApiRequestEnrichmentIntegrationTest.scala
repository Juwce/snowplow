...
object ApiRequestEnrichmentIntegrationTest {
  ...
  def createPair(key: SchemaKey, validJson: String): JsonSchemaPair = {
    val hierarchy = parse(
      s"""{"rootId":null,"rootTstamp":null,"refRoot":"events","refTree":["events","${key.name}"],"refParent":"events"}""").toOption.get
    (key, circeToJackson(Json.obj(
...
class ApiRequestEnrichmentIntegrationTest extends Specification with ValidationMatchers {
  ...
  def e2 = {
    ...
    context must beSuccessful.like {
      case context =>
        context must contain(beJson(IntegrationTests.correctResultContext2),
