...
class CacheSpec extends Specification with ValidationMatchers with Mockito {
  ...
  def e1 = {
    ...
    cache.put(key, Json.fromInt(42).success)
    cache.put(key, Json.fromInt(52).success)
    cache.get(key) must beSome.like {
      case v => v must beSuccessful(Json.fromInt(52))
  ...
  def e4 = {
    ...
    cache.put(key, Json.fromInt(33).success)
    cache.put(key, Json.fromInt(42).success)
    cache.get(key) must beSome.like {
      case v => v must beSuccessful(Json.fromInt(42))
    } and (cache.actualLoad must beEqualTo(1))
  }
}
