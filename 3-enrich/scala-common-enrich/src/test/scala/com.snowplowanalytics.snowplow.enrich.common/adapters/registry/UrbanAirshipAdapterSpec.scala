...
class UrbanAirshipAdapterSpec extends Specification with ValidationMatchers {
  ...
  "toRawEvents" should {
    ...
    val payload = CollectorPayload(
      Shared.api, Nil, None, validPayload.noSpaces.some, Shared.cljSource, Shared.context)
    ...
    "reject badly formatted json" in {
      val payload =
        CollectorPayload(
          Shared.api, Nil, None, """{ "value": "str" }""".some, Shared.cljSource, Shared.context)
      UrbanAirshipAdapter.toRawEvents(payload) must beFailing
    }

    "reject content types" in {
      val payload = CollectorPayload(Shared.api, Nil, "a/type".some, validPayload.noSpaces.some,
        Shared.cljSource, Shared.context)
