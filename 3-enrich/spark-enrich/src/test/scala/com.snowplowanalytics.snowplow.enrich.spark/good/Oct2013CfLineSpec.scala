 ...
 */
class Oct2013CfLineSpec extends Specification with EnrichJobSpec {
  import EnrichJobSpec._
  override def appName = "oct-2013-cf-lines"
  sequential
  "A job which processes a CloudFront file containing 1 valid page ping" should {
    runEnrichJob(Oct2013CfLineSpec.lines, "cloudfront", "1", false, List("geo"))
    ...
    "correctly output 1 page ping" in {
      ...
      for (idx <- Oct2013CfLineSpec.expected.indices) {
        actual(idx) must BeFieldEqualTo(Oct2013CfLineSpec.expected(idx), idx)
      }
    }
